...
func TestStats(t *testing.T) {
	cases := []struct {
		...
		{"single", []float64{7}, MetricStats{N: 1, Median: 7, P90: 7}},
		{"unsorted even", []float64{4, 2, 1, 3}, MetricStats{N: 4, Median: 2, P90: 4}},
		{"odd", []float64{1, 2, 3}, MetricStats{N: 3, Median: 2, P90: 3}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestStatsDoesNotMutateInput(t *testing.T) {
	in := []float64{3, 1, 2}
	got := stats(in)
	require.Equal(t, MetricStats{N: 3, Median: 2, P90: 3}, got)
