...
func buildAttrs(kind string, span *tracev1.Span, resourceAttrs []*commonv1.KeyValue) map[string]any {
	...
	case "assistant_turn":
		...
		if v, ok := lookupInt(attrs, "gen_ai.usage.cache_read_input_tokens", "gen_ai.usage.cache_read.input_tokens"); ok {
			m["cache_read_in"] = v
		}
		if v, ok := lookupInt(attrs, "gen_ai.usage.cache_creation_input_tokens", "gen_ai.usage.cache_creation.input_tokens"); ok {
			m["cache_write"] = v
		}
