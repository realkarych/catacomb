...
Four rules that are easy to forget and expensive to get wrong:
...
- **Always execute implementation plans subagent-driven, never inline** — dispatch a fresh subagent per task with a review after each task; the main window only orchestrates. Parallel implementer subagents are allowed — and encouraged — for tasks that touch disjoint files, each in its own isolated worktree; tasks that share files stay serial. Do not batch-execute plan tasks directly in the orchestrating session.
