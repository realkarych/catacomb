...
- Import captures no `artifacts/`: it runs no task `cmd`, so there is no live workdir to copy from. A `verify:` that reads a captured artifact therefore cannot score an imported cell — verifiers intended to run over imports must read the transcript/evidence dir, not `artifacts/`.
