...
|---|---|---|
| **Hooks** (`PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `SubagentStart`, `SubagentStop`, `Stop`, `UserPromptSubmit`, `SessionStart/End`, `PreCompact`, `PermissionRequest`, `Notification`) | Fire on **both** SDK and CLI paths; carry `session_id`, `tool_name`, `tool_input`, `tool_response`, `transcript_path`, MCP fields | Flat events; parent→child must be reconstructed |
...
```
 ...
 jsonl ───▶ │ JSONL tailer (fsnotify)      │    │  observation log (append)    │    └────────────────────────┘
            └──────────────────────────────┘    │      │                       │    ┌─ exporters ────────────┐
                                                 ...
                                                 │      │                       │    │ neo4j   (upsert + CDC)  │
                                                 │  durable store (SQLite/DuckDB)│   │ postgres(upsert + CDC)  │
                                                 └──────────────────────────────┘    └────────────────────────┘
...
Node {
  ...
  verbosity     enum{core, rich} // 'core' always present; 'rich' only in rich mode (lean mode = core only)
...
- `sequence` — temporal order within one agent's stream (optional, verbosity-gated).
...
- **WebSocket / SSE:** subscribe to a run (or all runs); receive an initial graph snapshot then a live **GraphDelta** stream. JSON envelope mirrors the export node/edge schema. Filters: run_id, node type, verbosity.
...
type Exporter interface {
    Name() string
    ApplyDelta(ctx, GraphDelta) error   // streaming (CDC)
    Snapshot(ctx, RunFilter) error      // on-demand full dump
}
```
...
- **CLI** (`catacomb`):
  ...
  - `export --to jsonl|neo4j|postgres [--run <id>] [--mode]` — snapshot/stream to a sink.
...
- **M0 — Core + hooks + jsonl.** `catacombcore` (model, observation log, reducer, SQLite store) + hook adapter + `install-hooks` + jsonl exporter + `replay`. Produces a graph from a live run.
- **M1 — OTel + reconciliation.** OTLP receiver + per-field precedence merge + provenance + CDC bus + **OTLP/OpenInference passthrough exporter** (free external visualization while building later milestones).
