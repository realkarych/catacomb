...
| `turn_context` + `event_msg:token_count` + `event_msg:task_complete` (grouped by `turn_id`) | `assistant_turn` | `MessageID: turn_id` | `model`, `tokens_in` (last_token_usage.input_tokens), `tokens_out` (output_tokens), `cache_read_in` (cached_input_tokens), `duration_ms` | Output = JSON-encoded final `message` role=assistant text of the turn, when present |
