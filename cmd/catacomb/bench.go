...
func newBenchCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runBench(cmd.OutOrStdout(), cmd.ErrOrStderr(), clientDiscoveryPath(), args[0], f)
		},
	}
...
func runBench(stdout, stderr io.Writer, discoveryPath, basketPath string, f benchFlags) error {
...
func printEpilogue(out io.Writer, b bench.Basket) {
	...
	fmt.Fprintf(out, "  catacomb baseline set %s-%s --label basket=%s,variant=%s\n", b.Name, first, b.Name, first)
	if len(b.Variants) >= 2 {
