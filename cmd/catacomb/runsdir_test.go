...
func TestRunsDirResolveGroupsAndFilter(t *testing.T) {
	root := evidenceRoot(t)
	base, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=base", loadForAggregation)
	...
	cand, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=cand", loadForAggregation)
	require.NoError(t, err)
	assert.Len(t, cand, 2)
}
...
func TestRunsDirEmptyMatchNamesSelector(t *testing.T) {
	root := evidenceRoot(t)
	_, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=none", loadForAggregation)
...
func TestRunsDirBadSelectorOperational(t *testing.T) {
	root := evidenceRoot(t)
	_, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "bogus", loadForAggregation)
	require.Error(t, err)
...
func TestRunsDirBadLabelTermOperational(t *testing.T) {
	root := evidenceRoot(t)
	_, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:BAD=x", loadForAggregation)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "invalid --label")
}

func TestRunsDirScanError(t *testing.T) {
	_, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", filepath.Join(t.TempDir(), "absent"), newPricer(), "label:variant=base", loadForAggregation)
	require.Error(t, err)
...
func TestRunsDirEvidenceLoadErrorPropagates(t *testing.T) {
	...
	_, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=base", loadForAggregation)
	require.Error(t, err)
}
...
func TestRunsDirNameSelectorMissingRunDir(t *testing.T) {
	...
	_, _, err := resolveSelectorRunsDir(io.Discard, "regress", dbPath, root, newPricer(), "name:golden", loadForAggregation)
	require.Error(t, err)
...
func TestResolveSelectorRunsDirForAggregationStripsHeavyNodeData(t *testing.T) {
	root := evidenceRoot(t)
	full, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=base", loadFullGraphs)
	require.NoError(t, err)
	...
	stripped, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=base", loadForAggregation)
	require.NoError(t, err)
...
func TestRunGroupFromDirsForAggregationStripsHeavyNodeData(t *testing.T) {
	...
	full, err := runGroupFromDirs("regress", root, "golden", ids, newPricer(), loadFullGraphs)
	require.NoError(t, err)
	payloads, _, _ := countHeavyNodeData(full)
	require.Positive(t, payloads)

	stripped, err := runGroupFromDirs("regress", root, "golden", ids, newPricer(), loadForAggregation)
	require.NoError(t, err)
	require.Len(t, stripped, len(full))
	payloads, attrs, sources := countHeavyNodeData(stripped)
	assert.Zero(t, payloads)
...
func TestRegressReportUnchangedByAggregationStrip(t *testing.T) {
	...
	reportJSON := func(mode runGraphLoadMode) string {
		base, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=base", mode)
		require.NoError(t, err)
		cand, _, err := resolveSelectorRunsDir(io.Discard, "regress", "", root, newPricer(), "label:variant=cand", mode)
		require.NoError(t, err)
