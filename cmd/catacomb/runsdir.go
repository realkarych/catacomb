...
func resolveSelectorRunsDir(errOut io.Writer, verb, dbPath, runsDir string, pricer reduce.Pricer, sel string) ([]aggregate.RunGraph, model.Baseline, error) {
	kind, val, err := parseSelector(sel)
	if err != nil {
		return nil, model.Baseline{}, operational(err)
	}
	if kind == selectorName {
		return resolveNameSelectorRunsDir(errOut, verb, dbPath, runsDir, pricer, val)
	}
...
func resolveNameSelectorRunsDir(errOut io.Writer, verb, dbPath, runsDir string, pricer reduce.Pricer, name string) ([]aggregate.RunGraph, model.Baseline, error) {
	...
	group, err := runGroupFromDirs(verb, runsDir, name, b.RunIDs, pricer)
	if err != nil {
...
func runGroupFromDirs(verb, runsDir, name string, ids []string, pricer reduce.Pricer) ([]aggregate.RunGraph, error) {
